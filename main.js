...
/* Creditos a Otosaka (https://wa.me/573132641948) */
...
const connectionOptions = {
    ...
    mobile: MethodMobile,
    browser: opcion === '1' ? ['ꜱᴀɴᴛx - ʙᴏᴛ ', 'Safari', '2.0.0'] : methodCodeQR ? ['ꜱᴀɴᴛx - ʙᴏᴛ ', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
    auth: {
        creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./Santx07Session")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./Santx07Session/${files}`)
})
} 
...
const directories = ['./Santx07Session/', './jadibts/']
...
if (reason == 405) {
await fs.unlinkSync("./Santx07Session/" + "creds.json")
